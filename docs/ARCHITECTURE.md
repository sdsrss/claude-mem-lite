...
| `hook.mjs` | 3431 | Hook v2 — Cognitive memory architecture Selective encoding, episodic batching, error-triggered recall Hooks (fast <100ms): post-tool-use,… | (entry — no exports) |
...
| `hook-llm.mjs` | 1590 | Background LLM workers for episode extraction and session summaries Extracted from hook.mjs for testability and reduced complexity | retractPreSavedObs(), recordRetryAttempt(), readRetryStats(), saveObservation(), persistHaikuSummary(), buildDegradedTitle(), saveEpisodeImmediate() … (+7) |
...
| `lib/fast-summary.mjs` | 420 | The non-LLM session summary — one shape, three callers. Audit 2026-08-22 P2-9. hook.mjs carried three hand-copied versions of "read this session's… | FAST_SUMMARY_LIMITS, readFastSummarySource(), insertFastSummary(), parseSummaryNotes(), formatSummaryNotes(), newestSummaryId(), writeStopSummary() … (+3) |
