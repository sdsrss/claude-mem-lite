...
function expandToken(token) {
  ...
  const parts = [token, ...capped].map((t) => (isSafe(t) ? t : `"${t.replace(/"/g, '""')}"`));
...
export function buildQueryFromText(text) {
  ...
  const embeddedEnTokens = (cleaned.match(/[a-zA-Z]{2,}/g) || []).map((w) => w.toLowerCase());

  const wsTokens = cleaned
    .split(/\s+/)
  ...
  for (const t of [...cjkTokens, ...embeddedEnTokens, ...wsTokens]) {
    ...
    if (!seen.has(key) && !TEXT_QUERY_STOP_WORDS.has(key)) {
      seen.add(key);
      tokens.push(t);
    }
  }
