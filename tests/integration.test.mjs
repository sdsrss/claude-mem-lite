...
describe('search pipeline integration', () => {
  ...
  it('full search flow: insert → FTS query → rerank → results', () => {
    ...
    insertObs(db, {
      title: 'auth session handling fix',
      text: 'auth session handling fix',
      filesModified: '["src/auth.js"]',
      importance: 2,
      epochOffset: -1000,
    });
    insertObs(db, {
      ...
      text: 'unrelated database migration',
    ...
    const docs = [
      {
        ...
        narrative: 'useEffect cleanup function prevents memory leaks in component lifecycle',
      },
      {
        title: 'react hooks memory management',
        narrative: 'useEffect and useCallback help manage component lifecycle and memory',
      },
      {
        title: 'react cleanup patterns',
        narrative: 'Component lifecycle cleanup with useEffect return function prevents leaks',
      },
    ];
    for (const d of docs) {
  ...
  it('pagination: no overlap between pages', () => {
    for (let i = 0; i < 30; i++) {
      insertObs(db, {
        title: `paginate test item ${i}`,
        text: `paginate test item ${i}`,
        epochOffset: -i * 1000,
      });
    }
...
describe('DB lifecycle', () => {
  ...
  it('init → seed → search → compress → verify invisible', () => {
    ...
    for (let i = 0; i < 5; i++) {
      insertObs(db, {
        title: `lifecycle old obs ${i}`,
        text: `lifecycle old obs ${i}`,
        importance: 1,
        accessCount: 0,
    ...
    for (const [, obs] of groups) {
      ...
      `,
        ...
        .run(
          'compress-test',
          'summary',
          `Weekly summary: ${obs.length}`,
          obs.map((o) => o.title).join('\n'),
          now.toISOString(),
          now.getTime(),
        );
  ...
  it('INSERT OR IGNORE handles concurrent session creation', () => {
    ...
    const count = db
      .prepare("SELECT COUNT(*) as c FROM sdk_sessions WHERE content_session_id = 'dup-sess'")
      .get();
  ...
  it('FTS triggers cascade on update', () => {
    ...
    // Update the title
    db.prepare(
      "UPDATE observations SET title = 'updated fts title', text = 'updated fts title' WHERE id = 1",
    ).run();
...
describe('tool flow: save → search → get roundtrip', () => {
  ...
  it('delete cleans up FTS entries', () => {
    insertObs(db, {
      ...
      title: 'deleteme unique findable',
      text: 'deleteme unique findable',
    });
    const id = db.prepare("SELECT id FROM observations WHERE title = 'deleteme unique findable'").get().id;

    // Verify FTS finds it
    const before = db
      .prepare(`SELECT rowid FROM observations_fts WHERE observations_fts MATCH '"deleteme"'`)
      .all();
