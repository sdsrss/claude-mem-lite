...
describe('registry enrichment confinement gate', () => {
  ...
  afterEach(() => {
    ...
    for (const d of [base, outside]) { try { rmSync(d, { recursive: true, force: true }); } catch { /* ignore */ } }
  });
  ...
  function seed(name, localPath) {
    db.prepare("INSERT INTO resources (name, type, status, source, local_path) VALUES (?, 'skill', 'active', 'user', ?)")
      .run(name, localPath);
  ...
  it('refuses a local_path outside the base and does NOT call the enricher', async () => {
    ...
    const res = await enrichResourceRow(db, row, { confineTo: base, enrichResource: spyEnricher(calls), env: {} });
  ...
  it('separates a missing path, an unreadable file and a refusal', async () => {
    ...
    const noPath = await enrichResourceRow(db, seed('nopath', ''), { confineTo: base, enrichResource: spyEnricher(calls), env: {} });
    expect(noPath.status).toBe('no-path');

    const gone = await enrichResourceRow(db, seed('gone', join(base, 'managed', 'does-not-exist.md')), { confineTo: base, enrichResource: spyEnricher(calls), env: {} });
  ...
  it('reports an enricher throw as failed, distinguishable from a refusal', async () => {
    ...
    const res = await enrichResourceRow(db, row, {
      confineTo: base,
      enrichResource: async () => { throw new Error('API 429'); },
      env: {},
    });
  ...
  it('the escape hatch actually admits the outside path', async () => {
    const row = seed('evil', outsidePath);
    const res = await enrichResourceRow(db, row, {
      confineTo: base, enrichResource: spyEnricher(calls), env: { [REGISTRY_CONFINE_ENV]: 'off' },
    });
    expect(res.status).toBe('enriched');
    expect(calls).toHaveLength(1);
  });
  ...
  it('throws rather than running ungated when confineTo is omitted — at all three entries', async () => {
    ...
    await expect(enrichResourceRow(db, row, { enrichResource: spyEnricher(calls) })).rejects.toThrow(/confineTo is required/);
    ...
    await expect(enrichImportedResources(db, [], { enrichResource: spyEnricher(calls) })).rejects.toThrow(/confineTo is required/);
    await expect(enrichNamedResource(db, 'no-such-name', { enrichResource: spyEnricher(calls) })).rejects.toThrow(/confineTo is required/);
    expect(calls).toEqual([]);
  });

  it('enrichNamedResource and enrichImportedResources carry the gate through', async () => {
    seed('evil', outsidePath);
    const named = await enrichNamedResource(db, 'evil', { confineTo: base, enrichResource: spyEnricher(calls), env: {} });
...
describe('every enrichment call site in both faces is gated', () => {
  ...
  function scanCallSites(src) {
    ...
    for (const fn of GATED_FNS) {
      ...
      while ((m = re.exec(src))) {
        ...
        if (/^\s*(export\s+)?(async\s+)?function\s/.test(src.slice(Math.max(0, m.index - 30), m.index))) continue;
  ...
  it('the scanner can return false — otherwise a green sweep proves nothing', () => {
    ...
    const gated = scanCallSites('await enrichNamedResource(rdb, name, { confineTo: DB_DIR, enrichResource });');
    expect(gated[0].gated).toBe(true);
    // A nested call must not let an inner argument list satisfy the outer one.
    const nested = scanCallSites('await enrichNamedResource(rdb, pick({ confineTo: X }), { enrichResource })');
  ...
  it('all four legs are present across the two faces', () => {
    const repo = dirname(dirname(fileURLToPath(import.meta.url)));
    const all = ['server.mjs', 'mem-cli.mjs'].flatMap(f => scanCallSites(readFileSync(join(repo, f), 'utf8')));
...
describe('CLI enrich refuses an out-of-base local_path (end to end)', () => {
  let dataDir, outside;
  beforeEach(() => { dataDir = makeTmpDir('cli'); outside = makeTmpDir('cli-out'); });
  afterEach(() => {
    for (const d of [dataDir, outside]) { try { rmSync(d, { recursive: true, force: true }); } catch { /* ignore */ } }
  });

  it('enrich <name> and enrich --all both refuse, naming the escape hatch', () => {
    const secret = join(outside, 'secret.md');
    writeFileSync(secret, '# must not be read\n');
    const imported = runCli(['registry', 'import', '--name', 'outside-skill', '--resource-type', 'skill', '--local-path', secret], dataDir);
