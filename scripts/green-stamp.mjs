...
//      blob ids), plus the node version. Untracked files are in the key because vitest
//      collects them too — tests/obs-id-caliber-sync.test.mjs emits a case per source file,
//      scratch included.
//
 ...
 * untracked-not-ignored path that exists as a regular file, with its blob id.
...
export function computeTreeKey(cwd, { nodeVersion = process.version } = {}) {
  ...
  const files = [...new Set(paths)].sort().filter((p) => {
    try {
      return lstatSync(join(top, p)).isFile();
    } catch {
      return false;
    }
  });
 ...
 */
export function checkStamp(cwd, { env = process.env } = {}) {
