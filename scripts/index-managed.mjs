...
const INTENTS = {
  ...
  deploy: [
    ...
    /\bship\b/i,
    /\bpublish\b/i,
    /continuous\s*(integration|delivery|deployment)/i,
  ],
  refactor: [
    /\brefactor\b/i,
    /\boptimize\b/i,
...
function extractFeatures(frontmatter, body) {
  ...
  const useWhenAll = desc.matchAll(
    /use\s+(?:this\s+)?(?:when|for|proactively\s*(?:when|for)?)\s+(.+?)(?:\.\s|$)/gi,
  );
  ...
  const verbObj = desc.match(
    /(?:review|debug|test|create|build|deploy|refactor|optimize|scaffold|analyze|design|implement|fix|diagnose|audit|check|monitor|migrate|upgrade)\s+\w+(?:\s+\w+)?/gi,
  );
  ...
  const whenSection = body.match(
    /(?:##\s*When\s+to\s+Use|##\s*Use\s+Cases?|##\s*When\s+This\s+(?:Skill|Agent)\s+Applies)\s*\n([\s\S]*?)(?=\n##\s|\n$)/i,
  );
  if (whenSection) {
  ...
  const caseSections = body.matchAll(
    /(?:##\s*(?:When|Use|Capabilit|What|Feature|Responsibilit)[^\n]*)\s*\n([\s\S]*?)(?=\n##\s|\n$)/gi,
  );
  ...
  if (/\bbeginner\b|\bsimple\b|\bbasic\b|\bstarter\b|\bquick\s*start/i.test(fullText))
    complexity = 'beginner';
  if (/\badvanced\b|\bexpert\b|\bmaster\b|\bsenior\b|\barchitect\b|\bdistributed\b|\bcomplex/i.test(fullText))
    complexity = 'advanced';

  // ── 9. Input/Output types ─────────────────────────────────────────────
  const inputTypes = new Set();
  if (/\bcode\b|\bsource\b|\bfile\b|\bfunction\b|\bclass\b|\bmodule\b/i.test(fullText))
    inputTypes.add('code');
...
function lookupRepoUrl(db, name, type) {
  try {
    const row = db
      .prepare('SELECT repo_url FROM preinstalled WHERE name = ? AND type = ?')
...
function main() {
  ...
  for (const col of newCols) {
    ...
    try {
      db.exec(`ALTER TABLE resources ADD COLUMN ${col} TEXT DEFAULT ''`);
    } catch {}
  }

  // Drop old FTS + triggers (may have wrong column order), rebuild with canonical order
  const triggers = db
    .prepare("SELECT name FROM sqlite_master WHERE type='trigger' AND tbl_name='resources'")
    .all();
  ...
  const upsertAll = db.transaction(() => {
    ...
    for (const r of allResources) {
      if (!indexedNames.has(`${r.type}:${r.name}`)) {
        db.prepare("UPDATE resources SET status = 'disabled', updated_at = datetime('now') WHERE id = ?").run(
          r.id,
        );
      }
    }
