...
export function registryConfineEnabled(env = process.env) {
  return (
    String(env[REGISTRY_CONFINE_ENV] ?? '')
      .trim()
      .toLowerCase() !== 'off'
  );
}
...
export function resourceUseHint(r, { home, managedPrefix }) {
  ...
  const portablePath = isManaged
    ? r.local_path.startsWith(home)
      ? '~' + r.local_path.slice(home.length)
      : r.local_path
    : '';
  ...
  if (isManaged) {
    ...
    howToUse =
      r.type === 'skill' ? `Skill("${r.invocation_name}")` : `Agent(subagent_type="${r.invocation_name}")`;
  } else {
