...
// cleared, access_count bumped towards boostAccessed. Measured 2026-09-25 over every
// top-level transcript on the authoring machine: 313 of 1877 `#NN` mentions in
// assistant text are dismissals, and the most-"cited" row of this repo (#54, 9x) was
...
const DISMISSAL_MARKER =
  /^[\s*`:：\-—–(（[]*(?:(?:本轮|本次|这次|此次|此处)\s*)?(?:n\/a\b|not applicable|does(?:n't| not) apply|did(?:n't| not) apply|irrelevant|not relevant|unrelated|不适用|不相关|无关|与(?:本|这|此)[^。\n；;]{0,16}无关)/i;
...
export function isDismissalAt(text, end) {
  ...
  if (DISMISSAL_MARKER.test(rest)) return true;
  const gloss = rest.match(DISMISSAL_GLOSS);
  return !!gloss && DISMISSAL_MARKER.test(rest.slice(gloss[0].length));
}
