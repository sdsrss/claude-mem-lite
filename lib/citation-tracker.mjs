 ...
 * longer move importance": `bumpCitationAccess` credits `access_count` for any
 * `#NN` in assistant text, and the `boost` maintain op lifts `importance + 1` for
 * `access_count > 3`. That is a SECOND, independent citation -> importance path
 * (obs #10911). It is untouched here because `access_count` has other writers
 * (explicit recall / get / timeline) and is also an input to noisePenaltyClause,
 * so changing it is a different decision with a different blast radius.
