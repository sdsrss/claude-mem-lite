...
// A P#/S#/E# anchor names a row that BELONGS to a project, and both surfaces document the
// conversion as "the nearest-in-time observation in the same project". It was resolved
// globally, which does not merely pick one odd row: fetchTimelineWindow below then scopes
// the whole before/after window to `project || anchorRow.project`, so one cross-project
// anchor moves the entire rendered timeline into another project with nothing marking it.
// Each source therefore reports its own project alongside the epoch. `user_prompts` has no
// project column — it reaches one through the session that owns the prompt, which is the
// same edge `sdk_sessions` already carries for every other prompt-scoped read.
const SOURCE_ROW_SQL = {
  prompt: `SELECT p.created_at_epoch AS created_at_epoch, s.project AS project
             FROM user_prompts p
             LEFT JOIN sdk_sessions s ON s.content_session_id = p.content_session_id
            WHERE p.id = ?`,
  session: 'SELECT created_at_epoch, project FROM session_summaries WHERE id = ?',
  event: 'SELECT created_at_epoch, project FROM events WHERE id = ?',
};
...
export function resolveAnchorToken(db, rawAnchor, { project = null } = {}) {
  ...
  if (parsed.source === 'prompt' || parsed.source === 'session') {
    ...
    const row = db.prepare(SOURCE_ROW_SQL[parsed.source]).get(parsed.id);
    if (!row)
      return {
        ok: false,
        error: { code: 'source-not-found', name: srcName, prefix: srcPrefix, id: parsed.id },
      };
    const nearest = nearestObservation(db, row.created_at_epoch, project ?? row.project ?? null);
    if (!nearest)
      return {
        ok: false,
        error: {
  ...
  if (parsed.source === 'event') {
    const row = db.prepare(SOURCE_ROW_SQL.event).get(parsed.id);
    if (!row)
    ...
    if (!nearest)
      return {
        ok: false,
        error: { code: 'no-obs-near', prefix: 'E#', id: parsed.id, scope: project ?? row.project ?? null },
      };
    return {
      ok: true,
      anchorId: nearest.id,
  ...
  const promptRow = db.prepare(SOURCE_ROW_SQL.prompt).get(parsed.id);
  const sessionRow = promptRow ? null : db.prepare(SOURCE_ROW_SQL.session).get(parsed.id);
  const hit = promptRow
  ...
  const nearest = nearestObservation(db, hit.row.created_at_epoch, project ?? hit.row.project ?? null);
  if (!nearest)
    return {
      ok: false,
      error: {
        ...
        id: parsed.id,
        srcName: hit.name,
        scope: project ?? hit.row.project ?? null,
      },
    };
  return {
    ok: true,
...
export function formatAnchorError(error, dialect) {
  ...
  switch (error.code) {
    ...
    case 'no-obs-near': {
      ...
      // NAME THE SCOPE. Since the anchor became project-scoped, this is the answer a caller
      // gets when the source row's own project holds no live observations — and without the
      // scope the message reads as "that id has no neighbours anywhere", which is a different
      // and wrong fact. `--project` is the lever, so say it.
      const where = error.scope ? ` in project "${error.scope}"` : '';
      const hint = error.scope ? ' — pass --project to search another' : '';
      return cli
        ? `[mem] No observations near ${error.prefix}${error.id}${suffix}${where}${hint}`
        : `No observations near ${error.prefix}${error.id}${suffix}${where}.`;
    }
