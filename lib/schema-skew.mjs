 ...
 * @param {string} dbPath
 * `spawn` is injectable for one reason: the branch below that handles a child which
 * produced no parseable stdout is reachable only from a NATIVE crash (binding-probe.mjs
 * documents the SIGSEGV shape that leaves stdout empty), which cannot be provoked from a
 * test. A defensive path nothing drives is the same dead-guard class this repo flags
 * elsewhere, so it gets a seam rather than an excuse.
 *
