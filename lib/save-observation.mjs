...
export function saveObservation(db, params) {
  ...
  //
  // hook-llm.mjs runs its own three-tier dedup over this table without the filter and
  // is NOT the same defect: its 7d/3d low-signal tiers are meant to keep matching rows
  // that auto-compress has retired (that is what stops "Modified package.json" from
  // re-accumulating), and it returns null rather than handing an id back to a caller.
