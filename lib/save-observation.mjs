...
// Every explicit `mem_save` writes its observation under this session-id prefix (below);
// every auto-captured observation carries a real content-session id instead (a uuid, never
// this prefix), and every row in the `events` table is auto-captured by construction (no
// manual write path reaches it — see persistHaikuSummary). `isManualSave` is the read-side
// counterpart, the single place that distinguishes "Daniel typed this" from "the background
// worker inferred this" for display and ranking. Single source of truth: re-typing the
// prefix at each read site is how the write and read sides silently drift.
// Module-private: only `isManualSave` and the write path below need the literal.
const MANUAL_SESSION_ID_PREFIX = 'manual-';

/** True when an observation's memory_session_id marks it as an explicit `mem_save`, not
 * an auto-captured episode. `events` rows have no memory_session_id at all — callers must
 * treat every event as auto-captured without calling this. */
export function isManualSave(memorySessionId) {
  return typeof memorySessionId === 'string' && memorySessionId.startsWith(MANUAL_SESSION_ID_PREFIX);
}
