...
function createExclusive(lockPath, payload) {
  ...
  try {
    ...
    try {
      ...
      return e2.code === 'EEXIST' ? 'exists' : 'error';
    }
  } finally {
    try {
      unlinkSync(tmp);
...
export function acquireLock(lockPath, { staleMs = DEFAULT_STALE_MS, now = Date.now } = {}) {
  ...
  if (first === 'ok') return makeRelease(lockPath, payload);
  if (first === 'error') return null; // permission / fs error → fail closed
  {
    let sampled;
    try {
      ...
      return createExclusive(lockPath, payload) === 'ok' ? makeRelease(lockPath, payload) : null;
    }
