...
//
// The summary INSERT used to also write a TF-IDF observation_vectors row in the same
// (caller-owned) transaction, so that FTS-miss queries falling back to vector recall could
// still reach compressed summaries (audit P6). Phase-2 removed the vector arm, so both the
// write and the recall path it served are gone; the summary is reachable by FTS alone.
