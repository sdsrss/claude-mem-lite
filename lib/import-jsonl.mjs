 ...
 * like a key) never matched itself and re-imported on every run. One home, scrubbed here, so
 * the two can no longer drift apart (R12 pre-ship review P3-4).
 */
function importedObsTitle(toolUse) {
  const detail = toolUse?.input?.command || toolEditPath(toolUse?.input) || '';
  return scrubSecrets(`${toolUse?.name || 'unknown'}: ${String(detail).slice(0, 80)}`);
}
...
export async function importJsonl(db, path, { project }) {
  ...
  const tryImportToolPair = (useEv, resultEv) => {
    ...
    // written and check the seenObs set seeded from the DB. Same function as the
    // storage site, so the scrubber cannot rewrite one and not the other.
