 ...
 * P2-2). Rows written before this carry `fast`, `llm`, '' or bare Failed / Uncertain text and
 * read as the least-protected provenance, so fresh data can replace them. `get` prints
...
export function writeStopSummary(db, { sessionId, project, report, source, now, limits }) {
  ...
  db.transaction(() => {
    ...
    if (done) {
      completed = done;
      prov.done = 'report';
    } else if (prov.done === 'titles' && titles) {
      completed = titles;
    }
    if (hasReport) {
      remaining = notDone;
 ...
 */
export function mergeModelSummary(db, { sessionId, project, fields, now }) {
  db.transaction(() => {
    const id = newestSummaryId(db, sessionId);
    if (id === null) {
      const last = db
      ...
      db.prepare(
        ...
        formatSummaryNotes({ done: 'model', left: 'other', lines: '' }),
        fields.lessons ?? null,
    ...
    ).run(...SUMMARY_COLUMNS.map((c) => next[c]), formatSummaryNotes(prov), id);
  }).immediate();
}
