...
//
//   • UserPromptSubmit / UserPromptExpansion — the plainText is injected verbatim,
//     so the model receives `{"suppressOutput":true,…}` as literal escaped text and
//     suppressOutput is never honoured.
//   • every other event — the ADDITIONAL-CONTEXT channel returns nothing for plain
//     text, so both receipts are dropped in silence.
//
...
//
// ── TWO CORRECTIONS, re-verified against the 2.1.260 bundle (2026-09-04) ─────────
//
// The list above named SessionStart as a plain-text-injecting event, and read as if
// plain text on any other event went nowhere at all. Both were wrong, and the second
// one would make hook-precompact.mjs look like dead code to the next reader.
//
// 1. SessionStart is NOT in the plain-text set. The stdout classifier (`f8e` in
//    2.1.260, `Hxi` in 2.1.233 — match on shape, the minified names are rebuilt every
//    release) turns plain text into an injectable answer for exactly two events:
//
...
//          : { answer: {}, … } }
//
//    and the SessionStart consumer reads `additionalContexts`, which comes off that
//    empty `answer`. This costs us nothing today only because hook.mjs SessionStart
//    goes through queueHookContext (an envelope); the CLAIM was still false.
//
...
//    summarizer at four call sites. So hook-precompact.mjs writing a bare
