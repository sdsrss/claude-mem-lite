...
export function normalizeCrossSourceScores(results, sourceKey) {
  ...
  // Explicit-save provenance floor (2026-09-26, cocina "tandoor" audit). An observation
  // saved by an explicit `mem_save` lost to five auto-captured events on a query it
  // genuinely matched, not because it was less relevant but because BM25's IDF term is
  // ~0 on a 2-row observations table (one term, one matching doc: log((2-1+.5)/(1+.5))
  // = log(1) = 0) — so the obs's raw score rounded to ~-0.00001, SINGLE_MATCH_BANDS read
  // that as a "grazing match" (ratio < 0.1 -> -0.25), and every multi-hit event
  // (normalized to roughly -1..-0.87 above) sorted ahead of it in the ascending merge.
  // That is a corpus-size artifact of the relevance math, not a signal that the manual
  // save is less on-topic than an auto-captured episode — the multipliers this file
  // warns not to casually retune (scoring-sql.mjs) are the wrong lever here, since they
  // scale a BM25 base that is itself ~0 on a small store. Floor is strictly better than
  // the best attainable band (-1.05) and the best multi-row normalization (-1.0), so a
  // matching explicit save is guaranteed to sort at or above every auto-captured row,
  // while non-matching manual saves (score never set, or already 0) are untouched.
  // Never DEMOTES: Math.min only tightens an already-worse score toward the floor.
  const MANUAL_SAVE_SCORE_FLOOR = -1.2;
  for (const r of results) {
    if (r[sourceKey] === 'obs' && r.is_manual && typeof r.score === 'number' && r.score < 0) {
      r.score = Math.min(r.score, MANUAL_SAVE_SCORE_FLOOR);
    }
  }
}
