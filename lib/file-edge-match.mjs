...
/**
 * Escape a string for use as a LIKE *literal* under `ESCAPE '\'`.
 *
 * The backslash must be escaped FIRST and it is not optional: under `ESCAPE '\'`
 * SQLite reads `\` + any character as that character taken literally, so an
 * un-doubled backslash is consumed silently and the pattern quietly stops matching
 * the string it was built from. A Windows path is nothing but backslashes, which is
 * how the events leg of scripts/pre-tool-recall.js returned zero rows on win32 while
 * looking correct (R12 audit, partition B-1) — it escaped `%` and `_` only.
 *
 * NOT the only copy in the repo, and the first draft of this comment claimed it was.
 * project-utils.mjs has its own `likeLiteral` for LIKE-matching a caller-supplied
 * PROJECT name (R10 P2-3: `--project '%'` matched every project). That one is already
 * correct — it escapes all three characters in one class, the form adopted here — and
 * the two stay apart on purpose: this module is a leaf that the ~30 ms cold-start
 * hook scripts import, and project-utils.mjs pulls a DB handle's worth of graph
 * behind it. Deliberate duplication, stated, rather than an unnoticed second copy of
 * the kind that produced B-1.
 *
 * Module-private: both callers are in this file. Export it when a second module
 * needs it, not before.
 */
