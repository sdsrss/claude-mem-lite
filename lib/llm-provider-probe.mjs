...
/**
 * Host and port the keyed provider actually serves from. The direct-API leg
 * honours ANTHROPIC_BASE_URL (haiku-client's callModelAPI), so probing
 * api.anthropic.com would certify a gateway deployment against a hop the
 * product never uses - the same false-green shape the proxy seam below exists
 * to avoid.
 * @param {'api'|'openrouter'} mode
