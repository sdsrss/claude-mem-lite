...
function ensureResourcesFts(db) {
  ...
  if (ftsRow) {
    ...
    const drifted =
      existingCols.length !== FTS5_COLUMNS.length || FTS5_COLUMNS.some((c, i) => existingCols[i] !== c);
    if (drifted) {
...
export function ensureRegistryDb(dbPath) {
  ...
  try {
    const resCols = new Set(
      db
        .prepare('PRAGMA table_info(resources)')
        .all()
        .map((c) => c.name),
    );
    if (!resCols.has('invocation_name'))
    ...
    if (!resCols.has('cooldown_hours'))
      db.exec('ALTER TABLE resources ADD COLUMN cooldown_hours INTEGER DEFAULT 0');
    // recommendation_mode: 'proactive' (default, actively recommended), 'on_request' (only when explicitly asked)
    if (!resCols.has('recommendation_mode'))
      db.exec("ALTER TABLE resources ADD COLUMN recommendation_mode TEXT DEFAULT 'proactive'");
    // weighted_adopt_sum: continuous adoption score accumulator (vs binary adopt_count)
    if (!resCols.has('weighted_adopt_sum'))
      db.exec('ALTER TABLE resources ADD COLUMN weighted_adopt_sum REAL DEFAULT 0');
    // Phase 2: Registry optimization columns
    if (!resCols.has('category')) db.exec('ALTER TABLE resources ADD COLUMN category TEXT');
    if (!resCols.has('quality_tier'))
      db.exec("ALTER TABLE resources ADD COLUMN quality_tier TEXT DEFAULT 'community'");
    if (!resCols.has('popularity_score'))
      db.exec('ALTER TABLE resources ADD COLUMN popularity_score REAL DEFAULT 0');
    if (!resCols.has('personal_score'))
      db.exec('ALTER TABLE resources ADD COLUMN personal_score REAL DEFAULT 0');
    if (!resCols.has('enrichment_status'))
      db.exec('ALTER TABLE resources ADD COLUMN enrichment_status TEXT DEFAULT NULL');
    if (!resCols.has('enriched_at'))
      db.exec('ALTER TABLE resources ADD COLUMN enriched_at INTEGER DEFAULT NULL');
    if (!resCols.has('repo_updated_at'))
      db.exec('ALTER TABLE resources ADD COLUMN repo_updated_at TEXT DEFAULT NULL');
    ...
    db.exec(
      "UPDATE resources SET quality_tier = 'installed' WHERE source = 'preinstalled' AND quality_tier = 'community'",
    );
  } catch (e) {
    debugCatch(e, 'resources-column-migration');
  }
  ...
  try {
    ...
    if (resSchema?.sql && !resSchema.sql.includes("'github'")) {
      ...
      try {
        db.transaction(() => {
          const hasOld = db
            .prepare(`SELECT 1 FROM sqlite_master WHERE type='table' AND name='resources_old'`)
            .get();
          ...
          const cols = db
            .prepare('PRAGMA table_info(resources_old)')
            .all()
            .map((c) => c.name);
    ...
    debugCatch(e, 'resources-source-check-migration');
  }
  ...
  if (resourcesRebuilt || ftsRecreated) {
    try {
      ...
      debugCatch(e, 'resources-fts-rebuild-after-source-check');
    }
  }
  ...
  try {
    const schema = db
      .prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='invocations'`)
      .get();
    if (schema?.sql && !schema.sql.includes('user_prompt')) {
      db.transaction(() => {
        // Clean up leftover from previous failed migration attempt
        const hasOld = db
          .prepare(`SELECT 1 FROM sqlite_master WHERE type='table' AND name='invocations_old'`)
          .get();
    ...
    debugCatch(e, 'ensureRegistryDb-migration');
  }
  ...
  try {
    ...
    debugCatch(e, 'ensureRegistryDb-ignored-migration');
  }
  ...
  try {
    ...
    debugCatch(e, 'rejection_reason-migration');
  }
  ...
  try {
    ...
    debugCatch(e, 'invocations-ondelete-cascade-migration');
  }
...
export function collectRegistryStats(rdb) {
  ...
  const byType = rdb
    .prepare('SELECT type, COUNT(*) as c FROM resources WHERE status = ? GROUP BY type')
    .all('active');
  const topAdopted = rdb
    .prepare(
  ...
  const zeroAdopt = rdb
    ...
    .get('active');
  const userAdded = rdb
    .prepare("SELECT COUNT(*) as c FROM resources WHERE status = ? AND source = 'user'")
    .get('active');
