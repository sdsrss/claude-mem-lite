...
- **The daily unattended `normalize` was the one path where ONE observation's content could
  ...
  **What the fan-out costs, stated rather than buried**: the default path no longer unifies
  ...
  old behaviour with `CLAUDE_MEM_NORMALIZE_CROSS_PROJECT=1`, which logs why that is a bad
  idea. Bounded to `NORMALIZE_MAX_PROJECTS_PER_RUN` (8) passes per run, since each is an LLM
  call where the union pass was one.
  ...
  **The remaining layers, each independently mutation-killed** (7 mutations, each asserted to
  have LANDED first): (1) `isConceptShaped` gates SHAPE where stored content becomes model
  input — length 2..40, a per-row cap of 32 so one row cannot monopolise the pool, and a
  **denylist** by Unicode CATEGORY (`Cc Cf Cs Co Cn Zs Zl Zp`) plus an NFKC fold, never a
  `\w` allowlist (ASCII-only in JS: it eats `café`). (2) `{system, user}` +
  `MEMORY_INPUT_GUARD`. (3) membership of the model's answer in **that project's** vocabulary,
  case-insensitively, matching `aliasMap`'s own lowercasing.
