...
- **Version**: 6.1.0 — **this exact string is a release guard.**
...
- **`package.json`'s `os` field is an npm INSTALL gate, and in plugin mode it sits on the
  ...
  proxy. **Before adding a manifest field that npm or a host enforces, ask which failure the
