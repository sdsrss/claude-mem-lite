...
- **Version**: 6.13.2 — **this exact string is a release guard.**
...
- **`ORDER BY created_at_epoch DESC` without an id tiebreaker INVERTS on a tie** — SQLite returns ascending rowid, i.e. oldest first, and two inserts share a millisecond **90.67%** — one population's rate, not a property (UPS/pretool: **0.00%**). 26 sites fixed; the rest **unjudged, not cleared**. Spelling: `importance DESC, created_at_epoch DESC, id DESC`.
