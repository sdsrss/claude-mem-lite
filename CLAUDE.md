...
- **The daily unattended `normalize` was the one path where ONE observation's content could
  ...
  **What the fan-out costs, stated rather than buried**: the default path no longer unifies
  ...
  old behaviour with `CLAUDE_MEM_NORMALIZE_CROSS_PROJECT=1`, which WARNS on stderr why that is
  a bad idea. **That flag is the only route back** — an explicit `optimize --run --task
  ...
  **The remaining layers, each independently mutation-killed** (11 mutations, each asserted to
  ...
  **denylist** by Unicode CATEGORY (`Cc Cf Cs Co Zs Zl Zp`) plus five characters that are Lo/So
  by category and BLANK on screen (U+3164 U+115F U+1160 U+FFA0 U+2800), and an NFKC fold judged
  against the punctuation class ONLY. Never a `\w` allowlist (ASCII-only in JS: it eats
  ...
  **Three things in that gate are deliberate and each was got wrong once.** U+200C ZWNJ and
  U+200D ZWJ are STRIPPED before the test rather than denied: they are required orthography in
  Persian and Hindi, and denying `\p{Cf}` wholesale made those concepts unrepresentable. The
  cost is that a phrase joined with them survives as one token — bounded by the fan-out to the
  attacker's own project. `\p{Cn}` was REMOVED: it binds the gate to the runtime's Unicode
  version, so the same token could be accepted on one Node and rejected on the next. And the
  NFKC fold is punct-ONLY, because applying the invisible classes to the folded form rejected
  `caf´e` — the keyboard spacing acute folds to space + accent and space is `\p{Zs}`, i.e. this
  bullet's own `café` example failing its own gate.
  **The 8-project cap ROTATES** (cursor in the gate file, resuming after the last project
  ...
  the CHANGELOG told users each one still was. **Its guard is deliberately a UNIT test**: the
  gate file is one file shared by every project and every concurrent run, so a "run twice and
  compare the picks" case passed alone and failed under vitest's parallel workers — a flake, not
  a guard.
  **The escape hatch warns via `console.error`, not `debugLog`** — the latter returns early
  unless `CLAUDE_MEM_DEBUG` is set, which the detached background worker does not set, so the
  documented safety net fired zero times in the only place it was ever needed.
