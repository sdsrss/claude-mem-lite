...
- **`ORDER BY created_at_epoch DESC` without an id tiebreaker INVERTS itself on a tie, and the
  ...
  `hook-optimize.mjs` only (all **six** sites — six, not the five a grep for the one-line form
  finds, because one is an `ORDER BY CASE …, created_at_epoch DESC` split across lines — plus the
  reduce, made total on its own because it is exported and callers build their own clusters).
  **The other ~42 sites in other files are NOT cleared, just unjudged** (D#15): most are display
  order, where an arbitrary tie is cosmetic. Match `hook-memory.mjs:683`'s spelling
  (`importance DESC, created_at_epoch DESC, id DESC`) — it is the one face that already got this
  right.
...
- **An `if (x)` guard whose else-branch is a LOOSER RULE is not a fallback, it is a second
  policy nobody reviewed.** `tfidf.mjs:419-425` states the design intent that a null
  vocabulary makes every vector path *skip* via its `if (vocab)` guard. `clusterForCompression`
  is the one site where the else instead groups by a **14-day window alone, with no
  similarity check** — and since `vectorsEnabled()` requires `CLAUDE_MEM_VECTORS === '1'`,
  that looser branch is the DEFAULT path, running unattended (`hook.mjs:1940` →
  `handleLLMOptimize` passes no `tasks`). Measured 2026-09-07 with a control arm, three
  observations sharing only project and era, 12 days apart: **arm on → 0 clusters, arm off →
  1 cluster of all three.** Not observed in production here (0 eligible rows on this
  corpus), so it is a demonstrated mechanism, not an incident. **And the two LLM cluster
