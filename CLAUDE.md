...
- **The daily unattended `normalize` was the one path where ONE observation's content could
  ...
  **THREE hand-drawn versions of that class each rejected real text, which is why it is now a
  ...
  of trusting the class name. `\p{Cn}` is absent because unassigned is a moving target that
  binds the gate to the runtime's Unicode version. The NFKC fold is punct-ONLY because applying
