...
| compress veto | `node benchmark/compress-veto-rate.mjs` · `--self-check` · `--json` | Whether D#10's `should_compress` veto FIRES, not just whether it exists. Two rates and both are required. Sends the SHIPPED prompt via `buildCompressPrompt`; classifies THREE ways so a dead key can never read as a perfect veto |
...
- **An `if (x)` guard whose else-branch is a LOOSER RULE is not a fallback, it is a second
  ...
  exactly the D#10 shape; it says nothing about ambiguous clusters, and an ambiguous arm is
  the named next step. **The ruler classifies THREE ways on purpose** — a refusal, a
