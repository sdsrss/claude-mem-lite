...
- **Version**: 6.0.0 — **this exact string is a release guard.**
...
- **The daily unattended `normalize` is the one path where ONE observation's content could
  rewrite rows in EVERY project, and it is now guarded in THREE places — removing any one
  of them re-opens it** (R10-P3-21 tier 3, fixed 2026-09-08). The chain, still true and
  worth knowing: `hook.mjs:3222` → `handleLLMOptimize` → `optimizeRun(db, {reenrichScope:
  'wide'})` **with no `project`** → `executeNormalize(db, force, {project: undefined})` →
  `extractUniqueConcepts` over every project → one prompt → `applyNormalization` writing
  every project. `applyNormalization`'s own comment says `--project` exists to stop exactly
  this and that NULL means "legacy unscoped run" — which is the mode the unattended caller
  uses. **That scoping was deliberately NOT changed**: making the unattended pass
  per-project is a released-artifact user-visible default change (§2-EXT checklist) and
  would kill cross-project `kubernetes`/`k8s` unification.
  **What bounds the attack, and why the gate is cheap**: `extractUniqueConcepts` splits on
  `/\s+/`, so a payload must survive as ONE whitespace-free token. Prose instructions
  cannot; a JSON group literal can, and needs `{ } [ ] " :`. Measured over three
  populations 2026-09-08 — real DB (1 row with concepts, 10 distinct), `seed-data.json`
  (200 rows, 541), `seed-data-cjk.json` (31 rows, 55) — **606 distinct real concept tokens,
  longest 22 (`infrastructure-as-code`), zero containing any of `{}[]"'` \` `\<>`.**
  The real-DB arm is far too small to calibrate on; the fixtures carry it.
  The three layers, each independently mutation-killed:
  (1) `isConceptShaped` gates SHAPE at the point stored content becomes model input — len
  2–40 plus a **denylist**, never a `\w` allowlist, because `\w` is ASCII-only in JS and an
  allowlist eats `café`; a literal hyphen must stay OUT of that class or
  `infrastructure-as-code` / `better-sqlite3` / `utf-8` are all filtered.
  (2) `{system, user}` + `MEMORY_INPUT_GUARD`, the split episode/summary already used.
  (3) membership: every `canonical` and `alias` must be in the input set,
  **case-insensitively**, matching `aliasMap`'s own lowercasing — a stricter check would
  reject groups `applyNormalization` would have applied.
  **Layer 3 is not redundant with 1 and 2**: removing it alone leaves a mocked model free to
  stamp an invented term across every project, and layers 1+2 do not catch that. Layer 2 is
  defense-in-depth wiring, not a behavioural guarantee (#8605: prompt wording barely moves
  the model), which is precisely why 3 exists.
  **`MEMORY_INPUT_GUARD` now lives in `lib/memory-input-guard.mjs`** because normalization
  made it a second consumer and a hand-copied security control is this repo's twin-drift
  class. **Do not merge it with `deep-search.mjs`'s `INJECTION_GUARD`** — different
  sentence, different input (the live query, not stored content), and #8729's import-weight
  reason still holds; `tests/memory-input-guard.test.mjs` pins the separation so the next
