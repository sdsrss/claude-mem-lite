...
|----------|-------|-------------|
| Tests | **359 files / 5936** (5935 passed, 1 skipped) | tag `v4.0.4`, 2026-09-06, **measured on the release tree immediately before the tag** (doctrine rule 7 — the `v4.0.1` row was stamped `tag v4.0.1` on numbers taken at `cb00974` plus an uncommitted worktree, which an independent review caught). v4.0.4 added 2 cases over v4.0.3's 359 / 5934 (one scope guard in `fast-summary`, one superseded case in `hook-context`); the third R8 §11.3 item was an in-place assertion rewrite, which adds no case, and no new test FILES, so `obs-id-caliber-sync`'s generated count is unmoved. v4.0.2 added 3 cases over v4.0.1's 359 / 5929; R8 itself added 2 test FILES and 18 cases (3 pool predicates + 2 Node floor + 4 link clearing + 9 repair hint). The `v4.0.0` figure was 357 / 5911, and was **byte-identical under vitest 4 and vitest 5 on the same tree** — that is the evidence the vitest bump was functionally neutral: the coverage denominators moved, the pass/fail set did not. (v4.0.0 was 357 / 5907 at tag `v3.99.0`: +4 cases, all in `native-binding-selfheal` for the source-compile heal; no new test FILES, so `obs-id-caliber-sync`'s generated count is unmoved) |
| Knip | **49** unused exports, **0** unused files, **0** duplicate exports | tag `v4.0.4`, primary working tree. Unmoved by v4.0.0 through v4.0.4, checked the right way each time: the unused-export NAME SET is **byte-identical**, not merely the same count (doctrine rule 4) — for v4.0.1 via a same-tree `git stash push -u` A/B, with the worktree diffed against its pre-stash state afterwards to prove the pop restored everything. R8's three new exports (`clearLinkPath`, `requiredNodeMajor`, `nativeBindingRepairHint`) are absent from the list because product code and tests import them. |
| Coverage | statements **84.18%** · branches **78.82%** · functions **89.28%** · lines **85.30%** | tag `v4.0.4`, **vitest 5.0.0**, 2026-09-06, release tree. The aggregate has not moved since v4.0.1 (v4.0.4 moved nothing either, and this time by construction: its one behaviour change is a single already-covered SQL line in `hook-context.mjs`, whose four columns 85.71 / 72.78 / 83.33 / 88.55 are themselves unmoved, and everything else in the release is comments, tests, and the CHANGELOG. v4.0.3 moved nothing at all — `hook-update.mjs` is outside the allowlist, and binding-probe's uncovered lines only renumbered again, 304 → 307); both releases moved exactly one row and both are attributed to `lib/binding-probe.mjs` — R8 funcs 62.5 → 64.7 (one added function its test covers), v4.0.2 branches 69.01 → 69.86 (the `sourceBuild === false` opt-out). Its uncovered lines only RENUMBERED both times (260 → 287 → 304) because docblocks pushed them down. `hook-optimize.mjs` and `install.mjs` are outside the coverage allowlist, so R8's other three fixes cannot move this row at all — check `vitest.config.mjs`'s `include` before attributing anything to a root module. Gate (80 / 74 / 84 / 83) passes, `test:coverage` exit 0. **Not comparable to any figure taken before the vitest 5 bump — see the caliber note below the next table.** The last vitest 4 reading, same tree, back-to-back: 84.30 / 78.82 / 89.27 / 85.40. **RE-MEASURE, NEVER CARRY — this row has been wrong twice.** The `v3.98.0` figure (84.34 / 78.88 / 89.26 / 85.44) did not reproduce at all: HEAD `e694259` itself measured 84.29 / 78.85 / 89.26 / 85.40, i.e. it had been carried across R6 ("unmoved by R5") instead of re-measured. **The ruler is per-tree reproducible** (A/A run twice on two different trees, byte-identical each time), so sub-0.05 moves are signal rather than run noise — which is exactly why carrying is not safe here. **UNATTRIBUTED, open since v3.99.0, do not guess a third time**: `lib/git-state.mjs` (100 / 90.9 / 100 / 100) is absent from `e694259`'s report and present afterwards, imported by nothing that changed and not pulled in by the new tests. Caliber note: the v8 text reporter truncates names past ~19 chars (`...n-tracker.mjs`), so a full-name grep returns nothing and reads as "not measured". |
